...
//  board[5][0].addPiece(new Queen(false, true, 900, 'Q', 0, 0));
    ...
    private int passedPawn(Piece pawn) {
        ...
        if (pawn.isWhite()) {
            for (int i = board.length - pawn.getRow(); i < board.length; i++) { //CHECK THIS CONDITION LATER? <--- should be rank minus row of pawn to check all pawns ahead of current pawn
        ...
        if (pawn.isWhite() == white) {
            passed += PASSEDPAWN_BONUS * (8-pawn.getRow()+1);
        } else {
